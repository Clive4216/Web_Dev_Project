...
function registerForEvent(eventId) {
    if (auth.currentUser !== null) {
        ...
        const eventRef = ref(database, `CommunityEvent/${eventId}`);
                ...
                if (userData && userData.username) {
                    ...
                    get(eventRef).then((snapshot) => {
                        ...
                        if (eventData) {
                            ...
                            set(ref(database, `CommunityEvent/${eventId}/Users/${userId}`), userData.username)
                                .then(() => {
...
function createMarkerPopup(eventId, event, url) {
    const popupContent = `
        <h1>${event}</h1>
